...
- **changelog** - update `CHANGELOG.md` with entries for the changes. After it completes, check if
  ...
  the relevant commit. **Only run when changes affect users**: files under `xirvik/`, `tests/`,
...
For example, if a commit contains `xirvik/commands/main.py`,
`tests/test_main_command.py`, and `CHANGELOG.md`, the component is determined by
`xirvik/commands/main.py` and `tests/test_main_command.py` only.
...
- Changes span unrelated components (e.g. `xirvik/media.py` and `.claude/agents/release.md`).
...
For Python files, strip the `xirvik/` prefix and replace `/` with `.` (like module imports).

- Python file `xirvik/media.py` → `media:`.
- Multiple files under `xirvik/commands/` → `commands:`.
- Single command file `xirvik/commands/admin.py` → `commands.admin:`.
