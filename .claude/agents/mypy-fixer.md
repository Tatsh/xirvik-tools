...
Fixes mypy type errors and eliminates `Any` usage in the xirvik-tools project.
...
1. Fix all mypy errors reported by `uv run mypy xirvik`
...
Place new TypedDict classes in `xirvik/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`xirvik/typing.py` when shared.
...
1. Run `uv run mypy xirvik` and capture all errors.
...
4. Run `uv run mypy xirvik` again to verify fixes.
