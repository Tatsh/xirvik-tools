---
name: python-expert
description: Expert-level Python author for non-trivial implementation work in the xirvik-tools project. Use when the task needs idiomatic, production-quality Python beyond a simple edit.
---

# Python Expert Agent

General-purpose expert-level Python coding agent for the xirvik-tools project.
...
- `` :py:mod:`xirvik.string` `` for modules
- `` :py:func:`xirvik.string.slugify` `` for functions
- `` :py:class:`xirvik.typing.ProbeDict` `` for classes
- `` :py:meth:`MyClass.my_method` `` for methods
- Use `~` to shorten the displayed name:
  `` :py:func:`~xirvik.string.slugify` `` renders as `slugify`
